...
double GOSIASimMinFCN::operator()(const double* par){
	...
	std::string	str_beam_inp;
	str_beam_inp = "./gosia < "+ beamGOSIAFile_inp+"> /dev/null";
	const char* 	c_b = str_beam_inp.c_str();
	system(c_b);		// 	Run the beam GOSIA file
	std::string	str_targ_inp;
	str_targ_inp = "./gosia < "+ targetGOSIAFile_inp+"> /dev/null";
	const char* 	c_t = str_targ_inp.c_str();
	...
	for(unsigned int i=0;i<exptData_Beam.size();i++){
		...
		for(unsigned int t=0;t<exptData_Beam.at(i).GetData().size();++t){
			...
			if(calcCounts > 0 && sigma > 0){
				if(verbose && ((index_init == 1 && index_final ==  0) || (index_init == 7 && index_final == 4))){
					if(fLikelihood){
